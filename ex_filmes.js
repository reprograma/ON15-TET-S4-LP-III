...
function encontrarPorTitulos(entrada) {
...
function encontrarPorGeneros(entrada) {
    let listaPorGenero = []
    entrada = entrada.toLowerCase()
...
function buscarOsFilmes(entrada) {
    if (typeof(entrada) == "string") {
        let porTitulo = encontrarPorTitulos(entrada)
        let porGenero = encontrarPorGeneros(entrada)
        if(porTitulo.length > 0 && porGenero.length > 0) {
            ...
            console.log("Nada foi encontrado. Por favor, digite novamente.")
        }    
    } else {
